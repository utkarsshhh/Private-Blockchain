...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if(await self.validateChain()==null){
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if(currentTime-time<300){
                if(bitcoinMessage.verify(message,address,signature)){
                    ...
                    let blockNew =  await self._addBlock(block)
                    resolve(blockNew)
                }
                else{
                    reject("error")
        ...
        return new Promise(async (resolve, reject) => {
            for(let i=0;i<self.chain.length;i++){
                if(!self.chain[i].BlockClass.validate()){
                    errorLog.push({'error':'Error with the Hash'})
                }
                else{
                    if(i>0){
                        if(self.chain[i].previousBlockHash!=self.chain[i-1].hash){
                            errorLog.push({'error':'Error in the previous hash'})
                        }
